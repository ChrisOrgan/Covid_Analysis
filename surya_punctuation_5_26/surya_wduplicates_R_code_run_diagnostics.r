...
dat$fitted_values <- 0.0000005895564 + 0.0000008291602*dat$node
...
dat_edit$fitted_values <- 0.0000005898981 + 0.0000008288184*dat_edit$node
...
ggsave("surya_wduplicates_figure_punctuation_diagnostics.pdf", width = 5,
       height = 4, device = cairo_pdf, plot_diag)
graphics.off()
ggsave("surya_wduplicates_figure_punctuation_diagnostics.svg", width = 5,
       height = 4, plot_diag)
graphics.off()
ggsave("surya_wduplicates_figure_punctuation_diagnostics_no_outliers.pdf",
       width = 5, height = 4, device = cairo_pdf, plot_diag_out)
graphics.off()
ggsave("surya_wduplicates_figure_punctuation_diagnostics_no_outliers.svg",
       width = 5, height = 4, plot_diag_out)
graphics.off()

# Write phylogenetic residuals to tab-delimited text files ----
dat_res_phy <- dat[, c(1, 5)]
dat_res_phy_edit <- dat_edit[, c(1, 5)]
write.table(
  dat_res_phy,
  file = "surya_wduplicates_R_output_phylogenetic_residuals.txt",
  quote = FALSE,
  sep = "\t",
  row.names = FALSE,
  col.names = FALSE
...
write.table(
  dat_res_phy_edit,
  file = "surya_wduplicates_R_output_phylogenetic_residuals_no_outliers.txt",
  quote = FALSE,
  sep = "\t",
  row.names = FALSE,
  col.names = FALSE
