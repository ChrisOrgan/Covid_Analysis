...
slope <- read.table("surya_cipres_R_output_subsampling_beta1.txt")
setwd("..")
...
plot_dist <-
  ggplot(slope, aes(slopes)) +
    ...
    annotate("text", x = 0, y = 380000, label = "") +
    geom_density(color = "black") +
    theme_tufte(base_size = 12, base_family = "Arial", ticks = FALSE) +
    labs(x = "\nSlope", y = NULL)

# Save scatter plots ----
CairoPDF("surya_cipres_figure_slope_distribution_subsampling.pdf",
         width = 6.535, height = 4.089)
print(plot_dist)
graphics.off()
CairoSVG("surya_cipres_figure_slope_distribution_subsampling.svg",
         width = 6.535, height = 4.089)
print(plot_dist)
graphics.off()
