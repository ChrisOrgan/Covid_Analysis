...
id <- read.table("sars_cov_2_2021_march_accession_gisaid.txt")

# Set partitions ----
part_start <- seq(from = 0, to = 450000, by = 10000)
...
for (set in 1:length(part$start)) {
  write.table(
    ...
    file = paste("sars_cov_2_2021_march_accession_gisaid_part_", set, ".txt",
                 sep = ""),
    quote = FALSE,
    sep = "\t",
    row.names = FALSE,
