...
# Part of the codes were adopted from Jacob Gardner.
...
for (subs in 1:1000) {
  ## Subset tree
  tip <- sample(dat$genome[dat$node > 1], size = 1)
  levels_back_max <- dat$node[dat$genome == tip] - 1
  ### The tree subset should not be the entire tree
  levels_back <- sample(1:levels_back_max, 1)
  tree_subs <- tree_subset(tree = tree, node = tip, levels_back = levels_back)
  while (
    length(tree_subs$tip.label) < 100 | length(tree_subs$tip.label) > 1000
  ) {
    tip <- sample(dat$genome[dat$node > 1], size = 1)
    levels_back_max <- dat$node[dat$genome == tip] - 1
    levels_back <- sample(1:levels_back_max, 1)
  ...
  path <- diag(vcv)
  time_temp <- dat[dat$genome %in% tree_subs$tip.label, c("genome", "time")]
  time <- time_temp$time
  ...
  sink(
    paste("sars_cov_2_subsampling_clade_output_r_node_density_artifact_", subs,
          ".txt", sep = "")
  )
  cat("======================\n")
  ...
  sink()
  sink(
    paste("sars_cov_2_subsampling_clade_output_r_regression_path_time_node_",
          subs, ".txt", sep = "")
  )
  cat("==============================\n")
  ...
  cat("\n")
  sink()
  sink("sars_cov_2_subsampling_clade_output_r_all_delta.txt", append = TRUE)
  cat(paste(list_delta[subs], "\n", sep = ""))
  sink()
  sink("sars_cov_2_subsampling_clade_output_r_all_beta0.txt", append = TRUE)
  cat(paste(list_beta0[subs], "\n", sep = ""))
  sink()
  sink("sars_cov_2_subsampling_clade_output_r_all_beta1.txt", append = TRUE)
  cat(paste(list_beta1[subs], "\n", sep = ""))
  sink()
  sink("sars_cov_2_subsampling_clade_output_r_all_beta2.txt", append = TRUE)
  cat(paste(list_beta2[subs], "\n", sep = ""))
  sink()
  sink("sars_cov_2_subsampling_clade_output_r_all_beta2_se.txt", append = TRUE)
  cat(paste(list_se2[subs], "\n", sep = ""))
  sink()
  sink(
    "sars_cov_2_subsampling_clade_output_r_all_beta2_p_val.txt",
    append = TRUE
  )
  cat(paste(list_p_val[subs], "\n", sep = ""))
  sink()
  sink(
    "sars_cov_2_subsampling_clade_output_r_all_beta2_partial_r2.txt",
    append = TRUE
  )
  cat(paste(list_partial_r2[subs], "\n", sep = ""))
  sink()
  sink("sars_cov_2_subsampling_clade_output_r_all_r2.txt", append = TRUE)
  cat(paste(list_r2[subs], "\n", sep = ""))
  sink()
  sink("sars_cov_2_subsampling_clade_output_r_all_var.txt", append = TRUE)
  cat(paste(list_var[subs], "\n", sep = ""))
  sink()
  sink(
    "sars_cov_2_subsampling_clade_output_r_all_punc_contrib.txt",
    append = TRUE
  )
  cat(paste(list_punc[subs], "\n", sep = ""))
  sink()
  sink("sars_cov_2_subsampling_clade_output_r_all_dbic.txt", append = TRUE)
  cat(paste(list_dbic[subs], "\n", sep = ""))
  sink()
  sink("sars_cov_2_subsampling_clade_output_r_all_n.txt", append = TRUE)
...
sink("sars_cov_2_output_r_regression_subsampling_clade_1000.txt")
cat("=====\n")
cat("Delta\n")
cat("=====\n\n")
...
sink("sars_cov_2_output_r_regression_subsampling_clade_1000.txt", append = TRUE)
cat("======\n")
cat("Beta 0\n")
cat("======\n\n")
